// src/features/alerts/components/AlertRuleForm.tsx
...
export const AlertRuleForm: React.FC<AlertRuleFormProps> = ({
  rule, onSubmit, onClose, loading,
}) => {
  const isEdit = !!rule;

  const [form, setForm] = useState<AlertRuleFormData>(
    rule ? ruleToFormData(rule) : { ...ALERT_RULE_FORM_DEFAULTS }
  );
  ...
  const setField = <K extends keyof AlertRuleFormData>(
    key: K, value: AlertRuleFormData[K]
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl bg-bg-primary shadow-2xl border border-border-default">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-default">
          <h2 className="text-lg font-semibold text-text-primary">
            {isEdit ? `Modifica regola` : 'Nuova regola di alerting'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-5">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-text-secondary opacity-70 mt-0.5">
                  Le regole disabilitate non generano alert
                </p>
              </div>
              <button
                type="button"
          ...
          <section className="flex flex-col gap-4">
            <h3 className="text-sm font-semibold text-text-primary border-b border-border-default pb-2">
              ...
              <span className="ml-2 text-xs font-normal text-text-secondary">
                (lasciare vuoto = qualsiasi valore)
              </span>
            </h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field
                label="Tipo evento (sottoCategoria)"
                hint="Es. login_failed, startup"
              >
                <input
                  type="text"
                  className={inputClass}
            ...
            <Field
              label="ID Origine"
              hint="Filtra per un servizio o utente specifico"
            >
              <input
                type="text"
                className={inputClass}
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <Field
                label="Conteggio eventi *"
                hint="1 = trigger immediato"
              >
                <input
                  type="number"
              ...
              >
                ...
                {errors.windowMinutes && (
                  <p className="mt-1 text-xs text-red-500">{errors.windowMinutes}</p>
                )}
              </Field>

              <Field
                label="Cooldown (minuti)"
                hint="Silenzio dopo un trigger"
              >
                <input
                  type="number"
                ...
                {errors.cooldownMinutes && (
                  <p className="mt-1 text-xs text-red-500">{errors.cooldownMinutes}</p>
                )}
              </Field>
            </div>
            ...
            <div className="rounded-lg bg-bg-secondary border border-border-default px-4 py-3 text-xs text-text-secondary">
              ...
              {form.cooldownMinutes > 0 && (
                <span> Successivi alert silenziati per {form.cooldownMinutes} minuti.</span>
              )}
            </div>
          </section>
