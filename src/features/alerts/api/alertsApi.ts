...
export const alertsApi = {
  ...
  async getRules(enabled?: boolean): Promise<AlertRule[]> {
    ...
    const data = await apiFetch<AlertRulesResponse>(
      `${BASE_URL}/rules${qs}`,
      { headers: readHeaders() }
    );
    return data.data;
  ...
  async getRule(id: string): Promise<AlertRule> {
    const data = await apiFetch<AlertRuleResponse>(
      `${BASE_URL}/rules/${id}`,
      { headers: readHeaders() }
    );
    return data.data;
  ...
  async getHistory(filters: AlertHistoryFilters = {}): Promise<AlertHistoryResponse> {
    return apiFetch<AlertHistoryResponse>(
      `${BASE_URL}/history${buildHistoryQuery(filters)}`,
      { headers: readHeaders() }
    );
  },
  ...
  async getHistoryStats(): Promise<AlertHistoryStatsResponse> {
    return apiFetch<AlertHistoryStatsResponse>(
      `${BASE_URL}/history/stats`,
      { headers: readHeaders() }
    );
  },
};
