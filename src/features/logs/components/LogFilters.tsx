      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-80 bg-bg-primary rounded-lg shadow-themed-lg border border-border-default z-50">
          ...
          <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFilters({ ...filters, categoria: (e.target.value as EventCategory) || undefined })
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFilters({ ...filters, criticita: (e.target.value as EventSeverity) || undefined })
                }
                className={selectClass}
              >
                <option value="">Tutte</option>
            ...
            <div>
              ...
              >
                <option value="">
                  {utentiLoading ? 'Caricamento...' : 'Tutti'}
                </option>
                {utenti.map((utente) => (
                  <option key={utente} value={utente}>{utente}</option>
                ))}
              </select>
            </div>
          ...
          <div className="flex items-center justify-end gap-2 p-4 border-t border-border-default">
            ...
            <Button onClick={handleApply} variant="primary" size="sm">Applica</Button>
          </div>
        </div>
      )}
    </div>
